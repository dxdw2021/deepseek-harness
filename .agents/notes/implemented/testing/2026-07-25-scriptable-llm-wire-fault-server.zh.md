...
服务器只报告协议层事实，不判断是否可重试。真实组合测试让请求依次经过 `dsh-llm-deepseek`、`dsh-agent-loop` 和 `dsh-llm-retry`：在当前默认策略下，连接遭拒、硬断开、部分输出后重置、空闲超时、合法的无内容完成以及正常关闭的部分输出 EOF 均可恢复（`STREAM_CLOSED` 在[被中断流决策](../bug-fix/2026-08-20-stream-closed-is-retryable.zh.md)中加入默认可重试 code）。该包不会改变这些策略。
...
**随服务器一起修改默认重试策略**：不予采纳。服务器用于揭示既有语义，而非决定策略。是否将恢复能力扩展到 `STREAM_CLOSED`，需要单独决策，并权衡成本、延迟和重复生成风险；该决策随后以默认可重试集合变更的方式交付，见[被中断流决策](../bug-fix/2026-08-20-stream-closed-is-retryable.zh.md)。
