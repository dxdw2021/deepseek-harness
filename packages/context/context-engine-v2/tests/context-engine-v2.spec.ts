...
import { ContextEngineV2Service } from '../src/index.ts'
 ...
 */
function makeBareContext(): Context {
  const ctx = new Context() as Context & { runtime: { effects: { bail: () => void }[] } }
  ...
  ctx.emit = ((event: string, ...args: unknown[]) => {
    ...
    return origEmit ? (origEmit as (event: string, ...args: unknown[]) => boolean)(event, ...args) : false
  }) as typeof ctx.emit
  ;(ctx as unknown as { on: (event: string, cb: (...args: unknown[]) => void) => () => void }).on = (event, cb) => {
...
function setup(): { ctx: Context; service: ContextEngineV2Service } {
...
describe('ContextEngineV2Service', () => {
  ...
  it('creates a memory entry from a compaction summary and prunes shadowed seqs', () => {
    ...
    const entry = service.handleCompactionSummary(
      [{ type: 'text', text: 'Compacted turn 1: did X then Y' }],
      [1],
      'deepseek',
      'deepseek-reasoner',
    )
