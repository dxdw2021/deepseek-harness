...
type AppearanceRowActions = {
  sync: (draft: AppearanceRowState, preference: ThemePreference, style: ThemeStyle, revision: number) => void
}
...
export function createAppearanceRowStore(): EngineStoreHandle<AppearanceRowState, AppearanceRowActions> {
  return defineStore({
    init: (): AppearanceRowState => ({ preference: 'system', style: 'graphite', revision: -1 }),
    actions: {
      sync: (d, preference: ThemePreference, style: ThemeStyle, revision: number) => {
