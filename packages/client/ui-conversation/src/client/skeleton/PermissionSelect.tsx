...
/** Map of preset values to locale keys for translated permission names. */
const PRESET_LOCALE_KEYS: Record<string, string> = {
...
function optionLabel(option: PermissionSelectValue['options'][number], t: ComposerBarProps['t']): string {
  const localeKey = PRESET_LOCALE_KEYS[option.value]
  return localeKey !== undefined ? t(localeKey as never) : displayName(option.name)
}
