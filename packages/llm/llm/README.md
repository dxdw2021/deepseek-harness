...
- `STREAM_CLOSED_CODE` — the provider-neutral code both adapters use for a stream that ended cleanly without its terminal marker (no `[DONE]` / done event). Classified as an error finish because the attempt cannot be trusted as complete; its failed chunks never enter derived messages, so `dsh-llm-retry` retries it by default.
