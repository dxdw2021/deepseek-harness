/**
 * Translate OpenCode Zen SSE payloads into the harness `StreamChunk` protocol
 * with one stateful harness block per content, reasoning, or tool-call index.
 * An empty initial reasoning delta does not open a block. Finish reason and
 * the latest usage are deferred until `[DONE]`, covering both finish-attached
 * and trailing usage-only shapes while ensuring no chunk follows `finish`.
 *
 * The wire dialect is the OpenAI chat stream, so the folding vocabulary (text,
 * `reasoning_content`, `tool_calls`, `finish_reason`, `usage`) matches any
 * OpenAI-compatible gateway.
 *
 * @module dsh-llm-opencode-zen/translate
 */
 ...
 * counts: OpenAI-style `prompt_tokens` INCLUDES cache hits, so cached reads
 * are subtracted out of `inputTokens`. Reasoning tokens are folded into
 * `outputTokens` by upstream providers; when reported separately they ride in
 * their own field.
